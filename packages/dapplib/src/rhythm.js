...
(async () => {
				...
				keyInfo.push({
					...
					weight:
						...
						Math.ceil(
							...
							? 1000
							: 500, // Half the keys will be 1000, the remaining 500
				});
			}
	...
	function getContractDependencies(contractType, filePath) {
		...
		while ((match = contractRegex.exec(code)) !== null) {
			contractNames.push(
				`${contractType}.${match.groups.contract}`
			);
		}
		...
		const importRegex =
			/\s*import\s+\S+\s+from\s+(?<import>\S+)\s+/gm;
